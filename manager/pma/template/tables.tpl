...
<?php 
    ...
    if(!$_POST['ok']) {
        ...
        echo "<br/>
            <form action='?act=multi&db=".urlencode($db_name)."' method='post'>
            <input type='hidden' name='do' value='".$_POST['do']."'>" . $_msg[1] .
            "<input type='submit' name='ok' value='".$lang->Yes."'>
            <a href='main.php'>".$lang->No."</a>
            </div>";
...
<form action='?act=multi&db=<?php echo urlencode($db_name); ?>' method='post'>
    ...
    <a href='table.php?tb=<?php echo urlencode($d[0])."&db=".urlencode($db_name); ?>'>
        <?php echo htmlentities($d[0]); ?>
    </a> <?php echo convert($d[6]+$d[8])." | ".$d[4]." ".($d[4]==1 ? $lang->row : $lang->rows);?> <br/>
