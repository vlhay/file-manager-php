...
if ($col_name == '' OR $check == 0) {
    header("Location: table.php?$_url");
    exit;
}
...
if ($act == 'drop') {
    ...
    if ($_POST['ok']) {
        ...
        if (!empty($col_data['Null']) && $col_data['Null'] != 'NO' && $col_data['Null'] != 'NOT NULL') $isnull = 1;

        // auto increment ?

        if (isset($col_data['Extra']) && strtolower($col_data['Extra']) == 'auto_increment') $isAI = 1;
