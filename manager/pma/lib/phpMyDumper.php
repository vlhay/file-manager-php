...

{
     ...
     */
    function __construct($dbn = null, $filepath = 'dump.php', $compress = false, $tables = false, $rds = false, $query = false)
    {
    ...
    function setDatabase($dbn)
    {
        global $db;
        $this->database = $dbn;
        if (!$db->select_db($this->database)) return false;
        return true;
    }
    ...
    function getTableStructure($table)
    {
        ...
        if ($this->dropTable) $structure.= "DROP TABLE IF EXISTS " . PMA_bkq($table) . ";\n";
        $records = $db->query("SHOW CREATE TABLE " . PMA_bkq($table));
        if ($records->num_rows == 0) return false;
    ...
    function getTableData($table, $hexValue = true)
    {
        ...
        if ($this->expInsert || $this->hexValue) {
            ...
            for ($x = 0; $x < $num_fields; $x++) {
                ...
                else $selectStatement.= '`' . $record['Field'] . '`';
        ...
        if (!$this->expInsert) $insertStatement = "INSERT INTO " . PMA_bkq($table) . " VALUES";
        if (!$this->hexValue) $selectStatement = "SELECT * FROM " . PMA_bkq($table) . " " . $this->rds;
        if ($this->query) {
    ...
    function doDump($doDB = false, $i = 1)
    {
        ...
        if ($this->phpMyAdmin) {
            ...
            if ($this->createTable) $this->getDatabaseStructure();
            if ($this->tableData) $this->getDatabaseData($this->hexValue);
        }
    ...
    {
        $file = false;
        if ($this->compress) $file = @gzopen($filename, "w9");
    ...
    {
        if ($this->outputTofile) {
            if ($this->compress) @gzwrite($file, $data);
