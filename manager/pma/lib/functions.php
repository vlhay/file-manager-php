        ...
        if (strcasecmp($match = $matches[0][$i], "") !== 0) {
            ...
            elseif (preg_match('/^[\s\n]+$/', $match)) // Space character(s)
            {
                ...
                if ($aKeyword) // Keyword found
                {
                    ...
                    if (is_numeric($match)) $buffer.= '<span class="numeric">' . htmlentities($match) . '</span>';
                    else
...
function getChar()
{
    ...
    while ($row = $res->fetch_assoc()) {
        if (!is_array($mysql_collations[$row['Charset']])) {
            $mysql_collations[$row['Charset']] = array(
                $row['Collation']
            );
        }
        else {
...
{
    foreach($_GET as $k => $v) {
        if (!in_array($k, $ignore)) $h.= "<input type='hidden' name='" . urlencode($k) . "' value='" . urlencode($v) . "'>";
    }
...
function getUniqueCondition($result, $row)
{
    ...
    foreach($fields as $k => $field) {
        ...
        if ($_primary_key) $primary[] = "(" . PMA_bkq($field->name) . " = '" . $db->real_escape_string($row[$field->name]) . "')";
        elseif ($_unique_key) $unique[] = "(" . PMA_bkq($field->name) . " = '" . $db->real_escape_string($row[$field->name]) . "')";
    }

    if ($primary) $result = implode(" AND ", $primary);
    elseif ($unique) $result = implode(" AND ", $unique);
    else $result = implode(" AND ", $c);
    return $result;
}
...
function isSqlFunction($str)
{
    ...
    if (in_array($str, $_var->sql_function_name)) return true;
