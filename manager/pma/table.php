...
if ($tb_name == '' OR $check == 0) {
    header("Location: tables.php?db=" . urlencode($db_name));
    exit;
}
...
elseif ($act == 'settings') {
    ...
    if ($_POST['newnm']) {
        ...
        if ($db->query($_q) !== TRUE) $_err = $db->error;
            ...
            $_SESSION['_q'] = $_q;
            header("Location: ?act=settings&db=" . urlencode($_POST['db_nm']) . "&tb=" . urlencode($_POST['newnm']) . "&ok=ionutvmi");
            exit;
        }
    }
    elseif ($_POST['copy']) {
        $_q = "";
        if ($_POST['type'] == 0 || $_POST['type'] == 1) $_q.= "CREATE TABLE " . PMA_bkq($_POST['db_nm']) . "." . PMA_bkq($_POST['copy']) . " LIKE " . PMA_bkq($db_name) . "." . PMA_bkq($tb_name) . ";";
        if ($_POST['type'] == 0 || $_POST['type'] == 2) $_q.= "SET SQL_MODE =  'NO_AUTO_VALUE_ON_ZERO'; INSERT INTO " . PMA_bkq($_POST['db_nm']) . "." . PMA_bkq($_POST['copy']) . " SELECT * FROM " . PMA_bkq($db_name) . "." . PMA_bkq($tb_name) . ";";
        if ($db->multi_query($_q) !== TRUE) $_err = $db->error;
    }
    elseif ($_POST['cl_nm']) {
...
elseif ($act == 'new') {
    ...
    if ($_POST['ok']) {
        ...
        if ($_POST['index'] != '---') {
            if ($_POST['index'] == "PRIMARY") $index = "PRIMARY KEY";
            else $index = ", ADD " . $_POST['index'] . " (" . PMA_bkq($_POST['name']) . ")";
        }

        if ($_POST['pos'] == 1) $pos = "FIRST";
        elseif ($_POST['pos'] == 2) $pos = "AFTER " . PMA_bkq($_POST['pos2']);
